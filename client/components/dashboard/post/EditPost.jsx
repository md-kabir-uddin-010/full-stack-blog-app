...
import {
  editPost,
  getPostById,
} from "../../../redux/features/post/actions/postAction";
...
export default function EditPost() {
  ...
  let initialValues = {
    ...
    slug: info?.post?.slug,
    keyword: info?.post?.keyword,
    description: info?.post?.description,
    schema: info?.post?.schema && JSON.parse(info?.post?.schema),
    category: info?.post?.category,
    thumbnail: info?.post?.thumbnail,
    post: info?.post?.post && JSON.parse(info?.post?.post),
    post_description: info?.post?.post_description,
    open_graph: {
      type: info?.post?.open_graph && JSON.parse(info?.post?.open_graph)?.type,
      title:
        info?.post?.open_graph && JSON.parse(info?.post?.open_graph)?.title,
      site_name:
        info?.post?.open_graph && JSON.parse(info?.post?.open_graph)?.site_name,
      site_url:
        info?.post?.open_graph && JSON.parse(info?.post?.open_graph)?.site_url,
      description:
        info?.post?.open_graph &&
        JSON.parse(info?.post?.open_graph)?.description,
      image:
        info?.post?.open_graph && JSON.parse(info?.post?.open_graph)?.image,
    },
    twitter_card: {
      card:
        info?.post?.twitter_card && JSON.parse(info?.post?.twitter_card)?.card,
      title:
        info?.post?.twitter_card && JSON.parse(info?.post?.twitter_card)?.title,
      site:
        info?.post?.twitter_card && JSON.parse(info?.post?.twitter_card)?.site,
      description:
        info?.post?.twitter_card &&
        JSON.parse(info?.post?.twitter_card)?.description,
      image:
        info?.post?.twitter_card && JSON.parse(info?.post?.twitter_card)?.image,
    },
    publish: info?.post?.publish,
  };
      ...
      >
        <Form>
          ...
          <div className=" mt-10 flex items-center justify-between">
            ...
            {/* <input
              className=" cursor-pointer text-base font-medium text-white capitalize py-2 px-5 hover:bg-[#1559ED] bg-[#1877F2] rounded-md break-all"
              type="submit"
              value="save"
            /> */}
          </div>
